...
h_plot_data <- h_data %>%
  mutate(Location = ifelse(Location == 0, 
                           "Dandenong", 
                           "Sunshine"
                           )
         )
...
### Equation
$$
\text{SalePrice} = `r round(multi_linear$coefficient[1], 2)` + (`r round(multi_linear$coefficient[2], 2)` \times \text{NumberOfRooms}) - (`r abs(round(multi_linear$coefficient[3], 2))` \times \text{Location}) + \varepsilon
$$
...
$$
\hat{\text{SalePrice}} = `r round(multi_linear$coefficient[1], 2)` + (`r round(multi_linear$coefficient[2], 2)` \times 9) - (`r abs(round(multi_linear$coefficient[3], 2))` \times 0) = 
$$
...
This also implies that including $\beta_\text{Number of Rooms}$ and $\beta_\text{Location}$ will have better predictive power than if the intercept was the only parameter.
...
joint_linear <- lm(SellingPrice~NumberofRooms+NumberRooms_Location, data = joint_h_data)
...
$$
\text{SalePrice} = `r round(joint_linear$coefficient[1], 2)` + (`r round(joint_linear$coefficient[2], 2)` \times \text{NumberOfRooms}) - (`r abs(round(joint_linear$coefficient[3], 2))` \times \text{NumberOfRooms} \times \text{Location}) + \varepsilon
$$

### Inferential Statistics with Interaction Term
```{r Linear Interaction Term Summary, comment = NA}

summary(joint_linear)
...
$H_0\text{: } \beta_{\text{Number of Rooms} \times \text{Location}} = 0$

$H_1\text{: } \beta_{\text{Number of Rooms} \times \text{Location}} \ne 0$
...
As an additional note, the AIC and BIC for the model with the interaction term (`r round(AIC(joint_linear), 2)` & `r round(BIC(joint_linear), 2)` respectively) are smaller than the no interaction term counterpart (`r round(AIC(multi_linear), 2)` & `r round(BIC(multi_linear), 2)` respectively), albeit by a minor margin, displaying minor improvements in model fit.
